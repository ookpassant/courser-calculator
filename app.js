...
(function () {
  ...
  const CHANGELOG = [
    { date: '9 Sep 2026', items: [
      ...
      'Smart Search was missing pairs it should have found. For most markings and modifiers it only credited a parent written the carrier way (nGi), so a horse with two copies (GiGi) or a shared-locus pair (GiCo, TRn, BFl) scored nothing for that trait and the pairing never appeared. It now reads each parent the same way breeding does, so any spelling that carries the allele counts. Girdle, Collar, Sabino, Roan, Tobiano, Blanched, Dominant White, Silver, Splash, Rabicano, Cuirass, Crowned, Shroud, Vellum, Tabard, Gilt, Pangare, Sooty and Illuminated all affected. False Leopard was worse: it was being read as the Leopard complex and never matched at all.'
    ] },
